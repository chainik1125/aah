...
class SpectrumSolver():
	...
	def _solve_quspin_blocks(self, symm_container, target_states):
		# Collect eigen-info across all symmetry blocks; keep sector labels to avoid shape issues
		energies_all: List[float] = []
		eigvecs_all: List[np.ndarray] = []
		nups_all: List[np.ndarray] = []
		ndns_all: List[np.ndarray] = []
		...
		for block in symm_container.iter_blocks():
			...
			# Accumulate per-eigenstate data with sector labels
			for idx in range(eigvecs.shape[1]):
				...
				eigvecs_all.append(psi_vec)
				nups_all.append(n_up_sites)
				ndns_all.append(n_down_sites)
				sectors.append((block.Nup, block.Ndn))

		if not energies_all:
			raise ValueError("No eigenstates obtained from symmetry blocks")

		# Sort globally by energy but keep sector labels alongside
		order = np.argsort(energies_all)
		energies_sorted = np.array([energies_all[i] for i in order])
		eigvecs_sorted = [eigvecs_all[i] for i in order]
		n_ups = np.stack([nups_all[i] for i in order], axis=0)
		n_downs = np.stack([ndns_all[i] for i in order], axis=0)
		n_tot = n_ups + n_downs
		sectors_sorted = [sectors[i] for i in order]

		# print(f'shape of energies_sorted: {energies_sorted.shape}')
		# print(f'len of eigvecs_sorted: {len(eigvecs_sorted)}')
		# print(f'shape of n_ups: {n_ups.shape}')
		# print(f'shape of n_downs: {n_downs.shape}')
		# print(f'shape of n_tot: {n_tot.shape}')


		return energies_sorted, eigvecs_sorted, n_ups, n_downs, n_tot, sectors_sorted
	...
	def solve_spectrum(self):
		if self.ham_lib=='tenpy':
			...
			if hasattr(ham, "iter_blocks"):
				...
				res = self._solve_quspin_blocks(ham, target_states)
				# Preserve sector labels for downstream use without changing the public return shape
				# if isinstance(res, tuple) and len(res) == 6:
				# 	energy_eigvals, energy_eigvecs, n_ups, n_downs, n_tot, sector_labels = res
				# 	self.last_sector_labels = sector_labels
				# 	return energy_eigvals, energy_eigvecs, n_ups, n_downs, n_tot
				return res
			else:
				raise ValueError("To support choosing arbitrary filling cheaply the code must solve each number block separately, otherwise have to rediagonalize.")
			# NOTE! I have code here before for the non-symmetry block case, but this will fail if I want to 
			# control filling cheaply. Look back in the git history if needed.
...
class FullSpectrum():
	...
	def set_mu_fix_filling(
		...
		total_spin_spectrum: np.ndarray,
		target_filling: Union[float, None] = None,
		temperature: Union[None, float] = 1e-8,
		mu_eff: Union[float, None] = None,
	):
		"""
		Docstring for set_filling_for_supercluster
		
		The function returns the appropriately formatted 
		Args:
			target_filling: Target filling for the supercluster
			total_energy_spectrum: Energy spectrum for the supercluster, shape (num_superclusters,states_retained)
			total_number_spectrum: Number spectrum for the supercluster, shape - notes its site-resolved! (num_superclusters,states_retained,supercluster_size)
			total_spin_spectrum: Spin spectrum for the supercluster, shape - note its site-resolved! (num_superclusters,states_retained,supercluster_size) 
		Returns:
			cluster_energy_expectations: shape (states_retained)
			cluster_number_expectations: shape (states_retained)
	...
	def get_cluster_thermodynamic_expectations(
		...
		set_filling: Union[float, None] = None,
		mu_eff: Union[float, None] = None,
	):
		...
		system_energy,system_number,system_spin=self.set_mu_fix_filling(
			full_energy_spectrum,
			full_number_spectrum,
			full_spin_spectrum,
			set_filling,
			temperature,
			mu_eff=mu_eff,
